    ...
    while doc.sections[0].start_type != max_pages:
        ...
        for para in doc.paragraphs:
            for run in para.runs:
                ...
                run.font.name = 'SimSun'  # 保持字体一致
                r = run._element
                r.rPr.rFonts.set(qn('w:eastAsia'), 'SimSun')  # 设置中文字体为宋体
...
def reformat_docx(file_path, output_path):
    ...
    # 设置左边距为正常的页面边距，右边距设为4.135英寸（半个A4宽度），实现左右对折后的排版
    section.left_margin = Inches(0.5)
    section.right_margin = Inches(4.135)  # 让内容只在左半部分显示
    section.top_margin = Inches(0.5)
    section.bottom_margin = Inches(0.5)

    # 添加文件名作为文档的第一段
    filename = os.path.basename(file_path).replace('.docx', '')
    first_para = doc.add_paragraph(filename)  # 插入文件名段落
    if first_para:
        first_para.alignment = WD_PARAGRAPH_ALIGNMENT.LEFT  # 左对齐
        first_para.runs[0].font.size = Pt(8)  # 设置为小六号字体
        first_para.runs[0].font.name = 'SimSun'  # 保持字体一致
